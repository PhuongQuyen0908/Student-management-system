...
const addStudentToClass = async ({
  ...
  try {
    ...
    if (
      !hs ||
      (hs.TrangThaiHoc?.localeCompare("Đang học", undefined, {
        sensitivity: "base",
      }) !== 0 &&
        hs.TrangThaiHoc !== null)
    ) {
      await t.rollback();
      const trangThaiHoc = hs ? hs.TrangThaiHoc : null;
      return buildResponse(
        `Học sinh có tình trạng học: ${trangThaiHoc}. Không thể thêm`,
        1,
        null
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (!ct) {
        await t.rollback();
        return buildResponse(
          "Không tìm thấy học sinh trong danh sách lớp",
          1,
          null
        );
      }
