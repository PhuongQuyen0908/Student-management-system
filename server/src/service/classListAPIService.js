...
db.bdmonhoc.hasMany(db.bdchitietmonhoc, {
  foreignKey: 'MaBDMonHoc',
  sourceKey: 'MaBDMonHoc',
});
...
const getAllStudentOfClass = async (
  MaDanhSachLop,
  page,
  limit,
  sortField,
  ...
  try {
    ...
    if (!lop || !namhoc) {
      return buildResponse(
        "Không thể truy xuất thông tin lớp hoặc năm học",
        1,
        []
      );
    }
    ...
    if (existing) {
      return buildResponse(
        "Danh sách lớp đã tồn tại, không thể tạo mới",
        1,
        existing
      );
    }
...
const deleteClassList = async (id) => {
  try {
    ...
    const deleted = await db.danhsachlop.destroy({
      where: { MaDanhSachLop: id },
    });
    if (!deleted) {
...
const addStudentToClass = async ({
  ...
  try {
    ...
    let classList = null;
    if (MaDanhSachLop) {
      classList = await db.danhsachlop.findByPk(MaDanhSachLop, {
        transaction: t,
      });
    }
    ...
    if (!classList) {
      ...
      const namhoc = await db.namhoc.findOne({
        where: { TenNamHoc },
        transaction: t,
      });
      if (!lop || !namhoc) {
    ...
    if (studentAlreadyInClass) {
      ...
      await t.rollback();
      return buildResponse(
        `Học sinh đã tồn tại trong danh sách lớp '${tenLop}' - Năm học ${tenNamHoc}. Không thể thêm`,
        1,
    ...
    if (!hs || hs.TinhTrang !== "Còn học") {
      await t.rollback();
      return buildResponse(
        "Học sinh không hợp lệ hoặc đã nghỉ học. Không thể thêm vào lớp",
        1,
        null
      );
    }
    ...
    const siSoToiDaParam = parameterList.find(
      (p) => p.TenThamSo === "SiSoToiDa"
    );
    ...
    if (classListLock.SiSo >= siSoToiDa) {
      await t.rollback();
      return buildResponse(
        `Lớp đã đạt sĩ số tối đa (${siSoToiDa}). Không thể thêm học sinh.`,
        2,
        null
      );
    }
    ...
    if (existingRecord) {
      await t.rollback();
      return buildResponse(
        "Học sinh đã tồn tại trong danh sách lớp. Không thể thêm",
        1,
        existingRecord
      );
    }
...
//chỉ xóa khi học sinh không có điểm trong bảng điểm
    ...
    try {
      ...
      for (const qth of quaTrinhHocList) {
        ...
        const bdMonHocList = await db.bdmonhoc.findAll({
          ...
          include: [
            {
              model: db.bdchitietmonhoc,
              required: true, // Nếu có ít nhất 1 dòng thì sẽ trả kết quả
            },
          ],
          transaction: t,
        });

        if (bdMonHocList.length > 0) {
         throw new Error("học sinh đã có dữ liệu bảng điểm."); // không rollback ở đây
        }
      }
       // Nếu không có điểm → tiến hành xóa
      for (const qth of quaTrinhHocList) {
